...
void AStarPathPlanner::__addNodesNearToOpenList(const Node& currentNode) {
  for (Node& potentialNode: this->allNodesInMap) {
    ...
    if (potentialNode.id < 0) {
      ...
      std::cout << "currentNode(2) = " << currentNode << "with newNode: " << *newNodePtr <<  std::endl;
      this->openList.emplace_back(newNodePtr);
      std::cout << "currentNode(3) = " << currentNode << "with newNode: " << *newNodePtr <<  std::endl;
