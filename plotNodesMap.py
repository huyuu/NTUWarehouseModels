...
pl.scatter(currentNode.iloc[:, 0], currentNode.iloc[:, 1], marker='+', s=80, color='green')
pl.scatter(trajectoryNodes.iloc[:, 0], trajectoryNodes.iloc[:, 1], marker='^', s=75, color='black')
# pl.scatter(startMap.iloc[:, 0], startMap.iloc[:, 1], marker='+')
pl.scatter(targetMap.iloc[:, 0], targetMap.iloc[:, 1], marker='v', color='orange')
pl.show()
